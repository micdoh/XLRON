"""Phase 3b: Run cut-set bounds keeping only the top 1% most congested cutsets.

Identical to Phase 3, but uses CUTSET_TOP_PCT=1 instead of CUTSET_TOP_K=256.
Results are saved with a _top1pct suffix to distinguish from the full run.
"""
...
def run_cutset_sweep(name, sweep_min, sweep_max, step, topo, sweep_file, timeout=14400):
    """Run cut-set bounds across a load sweep with top 1% cutset selection."""
...
def main():
    ...
    for topo in topologies:
        ...
        output_file = output_dir / f"{name}_top1pct.jsonl"
        existing_probes = {}
        ...
        print(progress.header(progress.processed + 1, name, f"({method}, top 1%)"))
        ...
        for sweep_num in range(MAX_SWEEPS):
            ...
            sweep_max = sweep_min + step * (N_POINTS - 1)

            print(f"  Sweep {sweep_num + 1}: [{sweep_min}, {sweep_max}] step={step}")

            sweep_file = sweep_dir / f"{name}_top1pct_sweep{sweep_num}.jsonl"
