...
def isrs_gn_model(
    ...
    attenuation_i: Array | None = None,
    attenuation_bar_i: Array | None = None,
    ch_power_W_i: Array | None = None,
    nonlinear_coeff: Array | None = None,
    ch_centre_i: Array | None = None,
    ch_bandwidth_i: Array | None = None,
    raman_gain_slope_i: Array | None = None,
    dispersion_coeff: Array | None = None,
    dispersion_slope: Array | None = None,
    coherent: bool = True,
    excess_kurtosis_i: Array | None = None,
...
def get_snr(
    ...
    max_spans: int = 20,
    ref_lambda: float = 1577.5e-9,
    attenuation_i: Array = 0.2 / 4.343 / 1e3 * jnp.ones(420),
    attenuation_bar_i: Array = 0.2 / 4.343 / 1e3 * jnp.ones(420),
    nonlinear_coeff: Array = 1.2 / 1e3 * jnp.ones(1),
    coherent: bool = True,
    mod_format_correction: bool = False,
    raman_gain_slope_i: Array = 0.028 / 1e3 / 1e12 * jnp.ones(1),
    dispersion_coeff: Array = 17 * 1e-12 / 1e-9 / 1e3 * jnp.ones(1),
    dispersion_slope: Array = 0.067 * 1e-12 / 1e-9 / 1e3 / 1e-9 * jnp.ones(1),
    roadm_loss: float = 6,
    num_roadms: int = 1,
    length: Array = 100 * 1e3 * jnp.ones(20),
    num_spans: int = 20,
    ch_power_w_i: Array = 10 ** (0 / 10) * 0.001 * jnp.ones(420),
    ch_centre_i: Array = ((jnp.arange(420) - (420 - 1) / 2) * 25e-9),
    ch_bandwidth_i: Array = 25e9 * jnp.ones((420, 1)),
    excess_kurtosis_i: Array = jnp.zeros((420, 1)),
    amplifier_noise_figure: Array = jnp.zeros((420, 1)),
    transceiver_snr: Array = jnp.zeros((420, 1)),
...
def get_snr_fused(
    ...
    amplifier_noise_figure: Array,
    transceiver_snr: Array,
