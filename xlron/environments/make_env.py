...
def make(
    ...
    if consider_modulation_format:
        ...
        if env_type == "rwa_lightpath_reuse":
            path_capacity_array = init_path_capacity_array(
                link_length_array,
                path_link_array,
                min_request=min(values_bw),
                R_s=100e9,
                scale_factor=scale_factor,
                alpha=alpha,
                NF=amplifier_noise_figure,
                beta_2=beta_2,
                gamma=gamma,
                L_s=span_length,
                lambda0=lambda0,
    ...
    # Convert setup arrays to JAX (kept as numpy during setup to avoid GPU round-trips)
    path_link_array = jnp.array(path_link_array, dtype=dtype_config.BINARY_DTYPE)
    if not isinstance(path_se_array, jax.Array):
        path_se_array = jnp.array(path_se_array, dtype=dtype_config.SMALL_INT_DTYPE)
    ...
    gap_starts = (
        HashableArrayWrapper(jnp.array(gap_start_slots))
        if not remove_array_wrappers
    ...
    if env_type == "vone":
        ...
        params_dict.update(
            ...
            last_fit=config.get("last_fit", False),
            max_power=max_power,
            min_power=min_power,
    ...
    if remove_array_wrappers:
        ...
        if env_type == "vone":
            env = VONEEnv(
                rng,
                params,
                virtual_topologies=virtual_topologies,
