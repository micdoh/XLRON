...
def _linear_apply(proj: Linear, x: Array) -> Array:
    """Apply an eqx.nn.Linear to a (possibly batched) input without vmap.

    Equinox Linear stores weight as (out_features, in_features) and computes
        y = weight @ x
    for vector x of shape (in_features,).

    For matrix/batched inputs of shape (..., in_features) we compute
        y = x @ weight.T
    yielding shape (..., out_features).

    This avoids generating a large number of small vmapped dot_generals, which XLA
    may fuse/reshape into a giant 2D dot (e.g. [720896, 128]) that can fail to tile
    under deterministic GPU lowering.
    """
    y = x @ jnp.swapaxes(proj.weight, -1, -2)
...
class MultiheadAttention(Module):
    ...
    !!! faq "FAQ"
        ...
        $d_\text{qk}$ is their "$d_k$". Our $d_\text{vo}$ is their "$d_v$". They
    ...
    def __call__(
        ...
        **Returns:**
        ...
        # Was: return jax.vmap(self.output_proj)(attn)
        # Avoid vmapping Linear over tokens; apply as a single matmul.
        return _linear_apply(self.output_proj, attn)

    def _project(self, proj, x):
        seq_length, _ = x.shape
        # Was: projection = jax.vmap(proj)(x)
        # Avoid vmapping Linear over tokens; apply as a single matmul.
        projection = _linear_apply(proj, x)
