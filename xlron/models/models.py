...
def make_dense_layers(x, num_units, num_layers, activation, layer_norm=False):
    ...
    layer = nn.Dense(
        ...
        param_dtype=dtype_config.PARAMS_DTYPE,
    )(x)
    layer = nn.LayerNorm(
        dtype=dtype_config.COMPUTE_DTYPE,
        param_dtype=dtype_config.PARAMS_DTYPE,
    ...
    def __call__(self, inputs):
        ...
        for size in self.feature_sizes:
            x = nn.Dense(features=size,
                         dtype=dtype_config.COMPUTE_DTYPE,
                         param_dtype=dtype_config.PARAMS_DTYPE,)(x)
            x = self.activation(x)
            x = nn.Dropout(rate=self.dropout_rate, deterministic=self.deterministic)(
                x
            )
            if self.layer_norm:
                x = nn.LayerNorm(dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)(x)
        return x
...
class LaunchPowerActorCriticMLP(nn.Module):
    ...
    def power_levels(self):
        ...
        return jnp.linspace(self.min_power_dbm, self.max_power_dbm, self.num_power_levels, dtype=dtype_config.SMALL_FLOAT_DTYPE)
    ...
    def __call__(self, x):
        ...
        def make_mlp(prefix):
            ...
            for i in range(self.num_layers):
                ...
                if self.layer_norm:
                    layers.append(nn.LayerNorm(name=f"{prefix}_norm_{i}", dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,))
            return layers
        ...
        if self.discrete:
            ...
            alpha_out = nn.Dense(1, kernel_init=orthogonal(0.01), name="alpha", dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)
            beta_out = nn.Dense(1, kernel_init=orthogonal(0.01), name="beta", dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)
        ...
        _, dist_params = nn.scan(
            ...
            length=self.k_paths,
        )()(None, jnp.arange(self.k_paths, dtype=dtype_config.MED_INT_DTYPE))

        # Initialize critic network layers
        critic_net = make_mlp("critic")
        critic_out = nn.Dense(1, kernel_init=orthogonal(1.0), name="critic_output", dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)
        ...
        for _ in range(self.message_passing_steps):
            ...
            if self.gnn_layer_norm:
                processed_graphs = processed_graphs._replace(
                    nodes=nn.LayerNorm(dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)(processed_graphs.nodes),
                    edges=nn.LayerNorm(dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)(processed_graphs.edges),
                    globals=nn.LayerNorm(dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)(processed_graphs.globals) if processed_graphs.globals is not None else None,
                )

        decoder = jraph.GraphMapFeatures(
            embed_global_fn=nn.Dense(self.global_output_size, dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,) if self.global_output_size > 0 else None,
            embed_node_fn=nn.Dense(self.node_output_size, dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,) if self.node_output_size > 0 else None,
            embed_edge_fn=nn.Dense(self.edge_output_size, dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,),
        )
...
class CriticGNN(nn.Module):
    ...
    def __call__(self, state: EnvState, params: EnvParams):
        ...
        state = state.replace(graph=state.graph._replace(globals=jnp.zeros((1, 1), dtype=dtype_config.LARGE_FLOAT_DTYPE)))
        ...
        critic = nn.Dense(1, kernel_init=orthogonal(1.0), bias_init=constant(0.0), dtype=dtype_config.COMPUTE_DTYPE, param_dtype=dtype_config.PARAMS_DTYPE,)(
            critic
        )
...
class ActorGNN(nn.Module):
    ...
    def __call__(self, state: EnvState, params: EnvParams):
        ...
        init_action_array = jnp.zeros(params.k_paths * self.edge_output_size, dtype=dtype_config.SMALL_FLOAT_DTYPE)
        ...
        if params.__class__.__name__ == "RSAGNModelEnvParams":
            if self.global_output_size > 0:
                ...
                init_feature_array = jnp.zeros((params.k_paths, edge_features.shape[1]), dtype=dtype_config.LARGE_FLOAT_DTYPE)
                # Define a body func to retrieve path slots and update action array
...
class ActorCriticGNN(nn.Module):
    ...
    def power_levels(self):
        """Calculate power levels dynamically"""
        return jnp.linspace(self.min_power_dbm, self.max_power_dbm, self.num_power_levels, dtype=dtype_config.LARGE_FLOAT_DTYPE)
    ...
    def sample_action_path(self, seed, dist, log_prob=False, deterministic=False):
        """Sample an action from the distribution."""
        action = jnp.argmax(dist.probs()).astype(dtype_config.MED_INT_DTYPE) if deterministic else dist.sample(seed=seed)
        if log_prob:
            return action, dist.log_prob(action)
        return action
