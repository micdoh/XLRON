...
- `--path_heuristic` - Algorithm: `ksp_ff`, `ksp_lf`, `ksp_bf`, `ksp_ef`, `ksp_mu`, `ksp_flf`, `ksp_flef`, `ksp_mscl`, `ff_ksp`, `lf_ksp`, `bf_ksp`, `mu_ksp`, `flf_ksp`, `mscl_ksp`, `kmc_ff`, `kmf_ff`, `kme_ff`, `kca_ff`, `arbr_ff`
  - Naming: `ksp_*` = try shortest path first; `*_ksp` = search all k paths jointly. Spectrum-assignment families in `heuristics.py`: exact-fit (`ksp_ef`), first-last-fit (`ksp_flf`/`flf_ksp`), first-last-exact-fit (`ksp_flef`), and minimum slot-continuity capacity loss (`ksp_mscl`/`mscl_ksp`, closed-form vectorised lookahead — the strongest and most expensive). `--mscl_interfering_k` sets the MSCL interfering routes per pair (1 = single-route 2013 formulation, default; 0 = all k = multi-route MSCL Sequencial/Combinado of dos Santos UFPE 2021). Shared helpers: `exact_fit`, `capacity_loss`, `is_large_request` in `heuristics.py`. `arbr_ff` is the ARBR adaptive path metric of Walkowiak et al. (JOCN 2018): `(1-α)·hops·slots + α·MLU` with `--arbr_alpha` (default 0.8), first-fit spectrum.
