...
SCRIPT_PATH="/Users/michaeldoherty/git/XLRON/xlron/train/train.py"
...
for weight in "--weight=weight" ""; do
  ...
  for k in $([ -n "$weight" ] && echo "5" || echo "5 50"); do
    ...
    run_experiment "DeepRMSA" "nsfnet_deeprmsa_directed" "250" "$k" "$weight" "--env_type rmsa --link_resources 100 --mean_service_holding_time 25 --truncate_holding_time"
    run_experiment "DeepRMSA" "cost239_deeprmsa_directed" "600" "$k" "$weight" "--env_type rmsa --link_resources 100 --mean_service_holding_time 30 --truncate_holding_time"
    ...
    for traffic_load in 168 182 196 210; do
        run_experiment "Reward-RMSA" "nsfnet_deeprmsa_directed" "$traffic_load" "$k" "$weight" "$args"
    done
    ...
    for traffic_load in 80 90 100 110 120 130 140 150 160; do
        run_experiment "MaskRSA" "nsfnet_deeprmsa_undirected" "$traffic_load" "$k" "$weight" "$args"
    done

    # MaskRSA JPN48
    for traffic_load in 120 130 140 150 160; do
        run_experiment "MaskRSA" "jpn48_undirected" "$traffic_load" "$k" "$weight" "$args"
    done
    ...
    for traffic_load in 180 190 200 210 220 230 240; do
        run_experiment "PtrNet-RSA-40" "nsfnet_deeprmsa_undirected" "$traffic_load" "$k" "$weight" "$args"
    done
    ...
    for traffic_load in 200 210 220 230 240; do
        run_experiment "PtrNet-RSA-80" "nsfnet_deeprmsa_undirected" "$traffic_load" "$k" "$weight" "$args"
    done

    # COST239 PtrNet-RSA-40
    args="$base_args --link_resources 40 --values_bw 1"
    for traffic_load in 340 360 380 400 420; do
        run_experiment "PtrNet-RSA-40" "cost239_ptrnet_real_undirected" "$traffic_load" "$k" "$weight" "$args"
    done

    # COST239 PtrNet-RSA-80
    args="$base_args --link_resources 80 --values_bw $var_bw"
    for traffic_load in 420 440 460; do
        run_experiment "PtrNet-RSA-80" "cost239_ptrnet_real_undirected" "$traffic_load" "$k" "$weight" "$args"
    done

    # USNET PtrNet-RSA-40
    args="$base_args --link_resources 40 --values_bw 1"
    for traffic_load in 210 220 230 240 250 260 270 280; do
        run_experiment "PtrNet-RSA-40" "usnet_ptrnet_undirected" "$traffic_load" "$k" "$weight" "$args"
    done

    # USNET PtrNet-RSA-80
    args="$base_args --link_resources 80 --values_bw $var_bw"
    for traffic_load in 260 270 280 290 300 310 320 330; do
        run_experiment "PtrNet-RSA-80" "usnet_ptrnet_undirected" "$traffic_load" "$k" "$weight" "$args"
    done
